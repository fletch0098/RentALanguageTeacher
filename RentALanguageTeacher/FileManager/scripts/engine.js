...
// Evento que recoloca las capas al cambiar de tamaño la ventana
...
// Inicialización. Generamos el arbol y controlamos el evento de cambio de tamaño de la ventana
...
function createTree() {
    ...
    $("#divTree")
		...
		.bind("remove.jstree", function (e, data) {
		    if (confirm("¿Está seguro de que desea borrar el directorio y todo su contenido?")) {
...
// Evento lanzado cuando se selecciona un directorio del arbol
function loadFiles(currentPath) {
    lastPath = currentPath;

    $("#iframeUpload").contents().find("input[type=text]").val(currentPath);

    $('#thumbs').html("<div class='thumbsLoader'><p>Buscando ficheros...<p><img src='/img/ajax-loader.gif' /></div>");
...
// Muestra los ficheros recibidos en la parte derecha
    ...
    $('div.thumb[ext!=jpg][ext!=JPG][ext!=bmp][ext!=BMP][ext!=png][ext!=PNG]').contextMenu("fileMenu", {
        ...
        bindings: {
            'delete': function (t) {
                ...
                if (confirm("¿Está seguro de que desea borrar el archivo?")) {
                    $.ajax({
                        url: 'ExplorerEngine.aspx',
...
function sendFileToCKEditor(fileUrl) {
    ...
    if (funcNum != null) {
        if (confirm("¿Desea seleccionar esta imágen?")) {
